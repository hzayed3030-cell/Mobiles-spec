...
def check_large_build_dirs(cwd: Optional[str] = None) -> List[Dict[str, Any]]:
    """فحص وجود فهارس build أو dist الخاصة بحزم ملفات EXE"""
    if cwd is None:
        cwd = os.path.dirname(os.path.abspath(__file__))
    found = []
    try:
...
def get_git_info(cwd: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not is_repo:
        return {
            ...
            "branch": "-",
            "remote": "-",
            "status_text": "المستودع المحلي غير مهيأ بعد (Not Initialized)",
            "has_changes": False,
            "last_commit": "-"
        }
    ...
    clean_remote = remote if (s_rem and remote and not remote.startswith("error")) else "-"
    
    return {
        "is_repo": True,
        "branch": branch if branch else "main",
        "remote": clean_remote,
        "status_text": status if status else "المستودع نظيف ومحدث",
        "has_changes": bool(status.strip()),
        "last_commit": last_c if last_c else "-"
    }

def set_git_remote(repo_url: str, cwd: Optional[str] = None) -> Tuple[bool, str]:
    """تعيين أو تحديث رابط المستودع البعيد (Remote Origin)"""
    if cwd is None:
        cwd = os.path.dirname(os.path.abspath(__file__))
    clean_url = repo_url.strip()
    if not clean_url:
        return (False, "رابط المستودع فارغ")
    
    is_repo, _ = run_git_cmd(["rev-parse", "--is-inside-work-tree"], cwd=cwd)
    if not is_repo:
        run_git_cmd(["init"], cwd=cwd)
        
    run_git_cmd(["remote", "remove", "origin"], cwd=cwd)
...
def export_project_to_github(
    ...
    if clean_url:
        ...
        if not s_push:
            ...
            logs.append(f"• git push -u origin {branch} (--force fallback): {push_out2 if push_out2 else 'تم الرفع بنجاح'}")
            if s_push2:
                return (True, "\n".join(logs))
        ...
        return (True, "\n".join(logs) + "\n⚠️ تم الحفظ محلياً بنجاح (يرجى إدخال رابط مستودع GitHub لرفعه على الإنترنت).")
...
def update_project_on_github(
    ...
    # 2. عمل Commit
    msg = commit_message.strip() if commit_message.strip() else "Update project features & specs"
    s, out = run_git_cmd(["commit", "-m", msg], cwd=cwd)
    logs.append(f"• git commit: {out if out else 'تم حفظ الـ Commit بنجاح'}")
    
    # 3. التأكد من اسم الفرع
    branch = branch_name.strip() if branch_name.strip() else "main"
    run_git_cmd(["branch", "-M", branch], cwd=cwd)
    
    # 4. التحقق من وجود Remote Origin
    s_rem, remote = run_git_cmd(["remote", "get-url", "origin"], cwd=cwd)
    if not s_rem or not remote or remote.startswith("error"):
        logs.append("⚠️ تنبيه: لا يوجد رابط Remote URL مرتبط حالياً. يرجى استخدام نافذة '🚀 Export to GitHub' لإدخال رابط مستودعك.")
        return (False, "\n".join(logs))
    
    # 5. محاولة سحب التحديثات ثم الرفع
